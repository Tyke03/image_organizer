...
## Project Overview
PhotoSort AI is a client-side React application that uses AI vision models to automatically analyze and organize photos into semantic albums based on visual content. The app uses OpenRouter as a unified API gateway to access multiple AI vision models with automatic fallback handling. The workflow is structured into modular tools for better maintainability and testability.
...
### AI/API Integration - OpenRouter Unified Gateway (Modular Tools)

#### Tool 1: Image Analysis & Tagging Tool (`services/imageAnalysisTool.ts`)
- **Purpose:** To analyze a single image and extract a concise description and relevant tags using OpenRouter.
- **Inputs:** Base64 encoded JPEG image data.
- **Outputs:** `{ description: string, tags: string[], modelUsed: string }`.
...
- **ALWAYS** use structured prompts requesting JSON output for consistent description and tag extraction.
...
#### Tool 2: Image Grouping & Clustering Tool (`services/clusteringService.ts`)
- **Purpose:** To organize a collection of images into distinct albums based on their extracted descriptions and tags.
- **Inputs:** A list of image metadata objects (containing `id`, `description`, `tags`).
- **Outputs:** A list of `Album` objects and a list of `ungrouped_image_ids`.
- **ALWAYS** run entirely client-side.
- **DO NOT** make external API calls for grouping logic.
- **ALWAYS** use algorithms to calculate similarity between tags and descriptions.
- **ALWAYS** implement logic to derive meaningful album names.
- **ALWAYS** handle images that don't fit into clear groups.
...
### Code Organization
- **ALWAYS** place utility functions in `/services` directory (imageUtils, imageAnalysisTool, clusteringService).
...
2.  **Modular Tools**: Core functionalities are isolated into independent, testable modules.
3.  **Sequential Processing**: Photos are analyzed one at a time to avoid API rate limits.
4.  **Adaptive Throttling**: Delay between API calls dynamically adjusts based on success/failure rates.
5.  **OpenRouter Unified Gateway**: Single API key with automatic multi-model fallback eliminates complex SDK management.
6.  **Smart Clustering**: Photos are grouped into albums of ~20 images using tag-based clustering with merge logic.
7.  **Graceful Degradation**: Failed photos are collected in a separate album with retry capability.
8.  **Model Tracking**: Each photo tracks which AI model successfully analyzed it for debugging and cost analysis.
...
-   **New AI Models**: Add to the `VISION_MODELS` array in `imageAnalysisTool.ts` - no new SDK needed.
